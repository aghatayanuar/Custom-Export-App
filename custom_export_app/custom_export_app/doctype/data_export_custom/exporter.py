...
def export_data(
	...
	template_bool = template.lower() == "true" if isinstance(template, str) else template
	export_without_column_meta_bool = (
		export_without_column_meta.lower() == "true"
		if isinstance(export_without_column_meta, str)
		else export_without_column_meta
	)
...
class DataExporter:
	...
	def build_response(self):
		...
		if self.file_type == "Excel":
			...
			if self.docname:
				self.save_as_attachment(self.docname)
				return
	...
	def save_as_attachment(self, docname):
		try:
			content = cstr(self.writer.getvalue())
			timestamp = now_datetime().strftime("%Y%m%d%H%M%S")
			filename = f"{self.doctype}-{timestamp}.csv"

			file_folder = frappe.get_site_path("private", "files")
			...
			frappe.db.sql("""
				INSERT INTO `tabFile` 
				(name, file_name, file_url, attached_to_doctype, attached_to_name, is_private, creation, modified)
				VALUES (%s, %s, %s, %s, %s, %s, NOW(), NOW())
			""", (frappe.generate_hash(), filename, file_url, "Data Export Custom", docname, 1))
			frappe.db.commit()

			frappe.db.set_value("Data Export Custom", docname, "exported_file", file_url)
			frappe.db.commit()
			...
			frappe.db.set_value("Data Export Custom", docname, "status", "Failed")
			frappe.db.commit()
			frappe.log_error(message=str(e), title="Export Failed")
			return None
	...
	def add_field_headings(self):
		...
		if self.export_without_column_meta:
			self.writer.writerow(self.labelrow[1:])
		else:
			self.writer.writerow(self.labelrow)
	...
	def add_data_row(self, rows, dt, parentfield, doc, rowidx):
		...
		if _column_start_end:
			for i, c in enumerate(self.columns[_column_start_end.start : _column_start_end.end]):
				...
				offset = 0 if self.export_without_column_meta else 1
				row[_column_start_end.start + i + offset] = value
	...
	def build_response_as_excel(self):
		...
		temp_csv = frappe.generate_hash(length=10) + ".csv"
		...
		if getattr(self, "docname", None):
			filename = f"{self.doctype}-{frappe.utils.now_datetime().strftime('%Y%m%d%H%M%S')}.xlsx"

			file_doc = frappe.get_doc({
				"doctype": "File",
				"file_name": filename,
				"content": xlsx_file.getvalue(),
				"attached_to_doctype": "Data Export Custom",
				"attached_to_name": self.docname,
				"is_private": 1,
			}).insert(ignore_permissions=True)

			frappe.db.set_value("Data Export Custom", self.docname, "exported_file", file_doc.file_url)
